...
class CountingThread(Thread):
    ...
    def run(self):
        ...
        while 1:
            ...
            sys.stdout.write(
                "   \033[132m[%s] Collected %s sandbox logs\033[1m   (Ctrl-C to view logs) [%s] seconds\r" % (sign, str(self.logs), str(time.time()-start_time).split(".")[0]))
            sys.stdout.flush()
            time.sleep(0.5)
            counter = counter + 1
            if self.stop:
...
def execute_droidbox(argv, logs_directory):
    ...
    p_mount = subprocess.Popen(["adb", "shell", "mount", "-o", "rw,remount", "rootfs", "/"])
    p_mount.wait()

    file_execute_strace = open("executeStraceIn.sh", "w")
    file_execute_strace.write("strace -p " + str(PID_ZYGOTE) + " -f -ttF -o /sdcard/" + apkName.split("/")[-1] + "_strace_monkey.txt")
    file_execute_strace.close()

    p_push = subprocess.Popen(["adb", "push", "executeStraceIn.sh", "/sdcard/"])
    p_push.wait()
    ...
    strace_process = subprocess.Popen(["adb", "shell", "sh", "./sdcard/executeStraceIn.sh"], stdout=subprocess.PIPE)
    ...
    subprocess_pull = subprocess.Popen(["adb", "pull", "/sdcard/" + apkName.split("/")[-1] + "_strace_monkey.txt"])
    subprocess_pull.wait()

    shutil.move(apkName.split("/")[-1] + "_strace_monkey.txt", logs_directory + "strace_" + apkName.split("/")[-1] + ".txt")
